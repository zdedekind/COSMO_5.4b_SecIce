...
!  4c. micorphysical process rates
  REAL  (KIND=wp), TARGET, ALLOCATABLE ::           &
    ...
    cmp_self_qns(:,:,:)     , &
    cmp_self_qng(:,:,:)     , &
    cmp_self_qnc(:,:,:)     , & ! Selfcollection of cloud droplets
    ...
    cmp_melt_qi_qr(:,:,:)   , & ! melting of ice particles
    cmp_melt_qs_qr(:,:,:)   , & ! melting of snow particles
    cmp_melt_ice(:,:,:)     , & ! melting of ice particles
    cmp_melt_snow(:,:,:)    , & ! melting of snow particles
    ...
    cmp_hom_sol_qni(:,:,:)  , &
    cmp_hom_qi(:,:,:)       , & ! Homogeneous freezing of cloud droplets
    cmp_hom_qni(:,:,:)      , & !
    cmp_evap_qc(:,:,:)      , & ! Change in QC after satuartion adjustment after microphysics
    cmp_sedi_qg(:,:,:)     , &
    cmp_sedi_qng(:,:,:)    , &
    cmp_sedi_qs(:,:,:)     , &
    cmp_sedi_qns(:,:,:)    , &
