...
! >> ZD
       &                       Ni_sec_gr_br_ice_d, Ni_sec_gr_br_snow_d, Ni_sec_dr_spl_d, &
       &                       Ni_sec_hm_d, Ni_pri_homhet_d,   &
! << ZD_20190819
...
!       &                       cmp_dep_qs_d, cmp_self_qnc_d, cmp_self_qi_d,&
       ...
       &                       cmp_melt_ice_d, cmp_melt_snow_d,       &
...
!            cmp_dep_qc_d(i,j,k)   = 0.0_wp
            ...
            cmp_dep_qg_d(i,j,k)   = 0.0_wp
            cmp_sub_qi_d(i,j,k)   = 0.0_wp
            cmp_sub_qs_d(i,j,k)   = 0.0_wp
            cmp_sub_qg_d(i,j,k)   = 0.0_wp
...
!           cmp_self_qnc_d(i,j,k) = 0.0_wp
            ...
            cmp_melt_snow_d(i,j,k)=0.0_wp
            cmp_melt_ice_d(i,j,k)=0.0_wp
            ...
            cmp_coll_qi_qg_d(i,j,k)=0.0_wp
            cmp_coll_qni_qng_d(i,j,k)=0.0_wp
...
!OCH<<
        Ni_sec_hm_d(i,j,k)=0.0_wp
        Ni_sec_dr_spl_d(i,j,k)=0.0_wp
        Ni_sec_gr_br_ice_d(i,j,k)=0.0_wp
        Ni_sec_gr_br_snow_d(i,j,k)=0.0_wp
!<< ZD_20190819        
...
!>> ZD 
        IF (cur_outstep .eq. ntstep-1) THEN
           Ni_sec_hm(ii,jj,kk) = 0
           Ni_sec_dr_spl(ii,jj,kk) = 0
           Ni_sec_gr_br_ice(ii,jj,kk) = 0 
           Ni_sec_gr_br_snow(ii,jj,kk) = 0
           Ni_pri_homhet(ii,jj,kk) = 0
        END IF
        Ni_sec_hm(ii,jj,kk) = Ni_sec_hm(ii,jj,kk) + Ni_sec_hm_d(i,j,k)
        Ni_sec_dr_spl(ii,jj,kk) = Ni_sec_dr_spl(ii,jj,kk) + Ni_sec_dr_spl_d(i,j,k)
        Ni_sec_gr_br_ice(ii,jj,kk) = Ni_sec_gr_br_ice(ii,jj,kk) + Ni_sec_gr_br_ice_d(i,j,k)
        Ni_sec_gr_br_snow(ii,jj,kk) = Ni_sec_gr_br_snow(ii,jj,kk) + Ni_sec_gr_br_snow_d(i,j,k)
        Ni_pri_homhet(ii,jj,kk) = Ni_pri_homhet(ii,jj,kk) + Ni_pri_homhet_d(i,j,k)
!<< ZD_20190819
...
!            cmp_dep_qc(ii,jj,kk)   = 0
            ...
            cmp_dep_qg(ii,jj,kk)   = 0
            cmp_sub_qi(ii,jj,kk)   = 0
            cmp_sub_qs(ii,jj,kk)   = 0
            cmp_sub_qg(ii,jj,kk)   = 0
...
!            cmp_self_qnc(ii,jj,kk) = 0
            ...
            cmp_melt_ice(ii,jj,kk)=0
            cmp_melt_snow(ii,jj,kk)=0
            ...
            cmp_coll_qi_qg(ii,jj,kk)=0
            cmp_coll_qni_qng(ii,jj,kk)=0
...
!          cmp_fr_qni(ii,jj,kk)       = cmp_fr_qni(ii,jj,kk) + cmp_fr_qni_d(i,j,k)
          ...
          cmp_dep_qg(ii,jj,kk)       = cmp_dep_qg(ii,jj,kk) + cmp_dep_qg_d(i,j,k)
          cmp_sub_qi(ii,jj,kk)       = cmp_sub_qi(ii,jj,kk) + cmp_sub_qi_d(i,j,k)
          cmp_sub_qs(ii,jj,kk)       = cmp_sub_qs(ii,jj,kk) + cmp_sub_qs_d(i,j,k)
          cmp_sub_qg(ii,jj,kk)       = cmp_sub_qg(ii,jj,kk) + cmp_sub_qg_d(i,j,k)
...
!          cmp_self_qnc(ii,jj,kk)     = cmp_self_qnc(ii,jj,kk) + cmp_self_qnc_d(i,j,k)
          ...
          cmp_melt_ice(ii,jj,kk)   = cmp_melt_ice(ii,jj,kk) + cmp_melt_ice_d(i,j,k)
          cmp_melt_snow(ii,jj,kk)   = cmp_melt_snow(ii,jj,kk) + cmp_melt_snow_d(i,j,k)
          ...
          cmp_coll_qi_qg(ii,jj,kk)   = cmp_coll_qi_qg(ii,jj,kk) + cmp_coll_qi_qg_d(i,j,k)
          cmp_coll_qni_qng(ii,jj,kk) = cmp_coll_qni_qng(ii,jj,kk) + cmp_coll_qni_qng_d(i,j,k)
