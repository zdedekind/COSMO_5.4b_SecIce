...
!  Use new module turb_data instead of data_turbulence, but set alpha1=0.0
!  Moved several variables from data_runcontrol to turb_data
!  Use several derived parameters from turbulence_utilities now
!
! Code Description:
! Language: Fortran 90 
...
USE data_runcontrol, ONLY:   &
    lseaice, llake, lsso, lconv, itype_turb, itype_tran, limpltkediff,       &
    l3dturb,                                                                 &
    ! but only for __COSMO__
    ntstep, lperi_x, lperi_y, l2dim, lscm

USE turb_data,       ONLY:   &
    ltkesso, ltkecon, icldm_tran, icldm_turb, imode_turb, itype_wcld,        &
    itype_sher, ltmpcor, lexpcor, lnonloc, lcpfluc,                          &
    zt_ice, z0_ice, len_min,             akt,         alpha0,                &
    alpha1, rlam_mom, rlam_heat, rat_lam, it_end, vel_min, tkesmot,          &
    c_scld, a_stab, tkhmin, tkmmin,                securi, wichfakt, c_diff, &
    clc_diag, q_crit, a_hshr, pat_len,                                       &
    d_m=>d_mom, a_h=>a_heat, a_m=>a_mom, d_h=>d_heat

USE meteo_utilities,       ONLY:   cloud_diag
USE turbulence_utilities,  ONLY:   stab_funct, turb_param, tet_g, rim, l_scal, &
                                   c_tke, l_hori, a_3, a_5, a_6, b_1, b_2,     &
                                   d_1, d_2, d_3, d_4, d_5, d_6, c_g
...
!     nur in LM-Umgebung:
      ...
      izerror  = 0
      yzerrmsg = '   '
      alpha1   = 0.0_wp    ! in new turb_data this is 1.0
