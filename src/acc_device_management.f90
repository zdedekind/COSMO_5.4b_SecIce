...
USE data_parallel,      ONLY: my_cart_id, num_compute, icomm_cart, imp_reals
...
! CUDA setup needs to be done _before_ MPI init
  ...
  READ(smyid,'(i8)') myid
  IF (debug) WRITE(*,*) 'DBG: MV2_COMM_WORLD_SIZE = ',numid
  IF (debug) WRITE(*,*) 'DBG: MV2_COMM_WORLD_RANK = ',myid
  ...
  ENDIF
  IF (debug) WRITE(*,*) 'DBG: COSMO_NPROC_NODEVICE = ',numnodev

  ! check if rank should init device
  IF (myid < numid - numnodev) THEN
    CALL setAccDevice()
  ELSE
    IF (debug) WRITE(*,*) 'DBG: excluding rank ', myid
  ENDIF
...
!!$  endif
  ...
  ELSE
    ...
    IF (ierr /= 0) THEN
      ...
      STOP
    ENDIF
    IF (debug) WRITE(*,*) 'DBG: gathered hostids'
    ...
    DO i = 2, numid
      ...
      ENDIF
    ENDDO
    IF (debug) WRITE(*,*) 'DBG: number of nodes = ',numnode
...
!------------------------------------------------------------------------------
  ...
  IF (numdev < 1) THEN
    ...
    STOP
  ENDIF
  IF (debug) WRITE(*,*) 'DBG: rank/#dev = ',myid,numdev
  ...
  IF (debug) WRITE(*,*) 'DBG: rank/dev = ',myid,mydev

  ! run small example kernel
  CALL runSmallAccKernel()
  IF (debug) WRITE(*,*) 'DBG: ran small init kernel'
...
!------------------------------------------------------------------------------
  ...
  zMemUsage = REAL(itotal-ifree,wp)/(1024.0_wp)**3

  IF (num_compute > 1) THEN
    CALL global_values(zMemUsage, 1, 'MAX', imp_reals, icomm_cart, 0, &
         yzerrmsg, izerror)
  END IF
  IF (my_cart_id == 0) &
    WRITE(*,"(A,A,A,F8.2,A)") 'INFO: GPU Memory usage (Max PE), ', &
    TRIM(mtag), ' : ',zMemUsage,' GB'
...
END MODULE acc_device_management
