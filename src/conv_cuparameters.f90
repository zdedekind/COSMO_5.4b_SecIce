...
MODULE conv_cuparameters

#ifdef __COSMO__
  USE kind_parameters, ONLY :  &
    & jprb=>wp,  &    !! KIND-type parameter for real variables
    & jpim=>i4        !! KIND-type parameter for standard integer variables
  USE conv_data,       ONLY :  &
    & t_phy_params,                                     &
    & tune_entrorg, tune_rhebc_land, tune_rhebc_ocean,  &
    & tune_rcucov, tune_texc, tune_qexc,                &
...
#ifdef __COSMO__
WRITE(*,*)'SUCUMF: RESOLUTION=',rsltn
!WRITE(*,*)'SUCUMF: KSMAX=',KSMAX
WRITE(*,FMT='('' COMMON YOECUMF '')')
WRITE(*,FMT='('' LMFMID = '',L5 &
  & ,'' LMFDD = '',L5,'' LMFDUDV = '',L5)') &
  & phy_params%lmfmid,lmfdd,lmfdudv
#endif
...
!------------------------------------------------------------------------------
  ...
  SUBROUTINE suvdfs
    ...
    !USE  conv_cufunctions , ONLY : PHIHS,PHIMS
