...
!  Replaced data_parameters by kind_parameters and used i8 for long integers
!
! Code Description:
! Language: Fortran 90.
...
USE kind_parameters, ONLY :   &
    ...
    i8           ! KIND-type parameter for long integer variables
...
! ENDIF
  ...
  IF (my_cart_id == 0) THEN
    ! get actual UTC-time
    !--------------------
    CALL get_utc_date (0, ydate_ini, dt, itype_calendar,          &
...
!------------------------------------------------------------------------------
  ...
  IF (my_cart_id == 0) THEN
    ...
    IF ( (nlines >= 48) .OR. (ntstep == nstop) ) THEN
      IF ( (MOD(NINT(ntstep*dt, i8),3600_8) == 0) .OR. (ntstep == nstop) ) THEN
        ...
        nlines  = 0
        realtot_new = 0.0_wp
        WRITE (ychh,'(I10)') NINT ( ntstep * dt / 3600.0_wp, i8 )
...
SUBROUTINE mean_testsuite (ntlev, ntke)
...
INTEGER , INTENT(IN) ::                                             &
  ...
  ntke               ! timelevel to output for TKE
...
!------------------------------------------------------------------------------
  ...
  ! For TKE we have to use ntke as time level
  IF ( lalloc_tke      ) CALL print_testsuite ("TKE", tke(:,:,:,ntke))
