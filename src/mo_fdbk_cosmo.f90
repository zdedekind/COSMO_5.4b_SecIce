...
USE kind_parameters,  ONLY: sp, wp
...
!     IF (file% nc% vars(in)% xtype == NF_CHAR)  kcase = 5
      ...
      IF (kcase == 1) THEN
        ...
        CASE ('ct_nwc')
          ivalh (1:nrep) = report(1:nrep)% header% ct_nwc
        CASE DEFAULT
...
!       =============
      ...
      ELSEIF (kcase == 2) THEN
        ...
        CASE ('ch_nwc')
          rvalh(1:nrep) = report(1:nrep)% header% ch_nwc
        CASE DEFAULT
...
!         SELECT CASE (file% nc% vars(in)% name)
          ...
          CASE ('dlat')
            rvalb (kos+1:kos+nobs) = report(irep)% body(1:nobs)% dlat
          CASE ('dlon')
            rvalb (kos+1:kos+nobs) = report(irep)% body(1:nobs)% dlon
          CASE ('w_qc')
...
!     file% nc% vars(in)% p(2)% dim% len = file% n_veri
      ...
      !    it is not assumed any more (in t_acc_body) that (file% n_veri <= 1)
      DO jj = 1, file% n_veri
        kos = 0
        DO irep = 1, nrep
          nobs = report(irep)% len
      ...
      ENDDO
      CALL add_data ( file, rvala2, in, 1, start, count, jerr )
!     =============
      DEALLOCATE(rvala2)
