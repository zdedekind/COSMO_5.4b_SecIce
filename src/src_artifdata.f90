...
!    rhsurf(:,:) = 0.95 + (0.03 - 0.95)/(1+EXP(-0.0015*(hsurf(:,:)-6000.)))
    ...
    !ZD: overwrite rel.hum with muhlbauer profile
    !RH9
    DO k=1, nlay-1
        relhum(:,:,:) = 0.85 + (0.03 - 0.85)/(1+EXP(-0.0007*(zml(:,:,:)-5000.)))
    END DO
    rhsurf(:,:) = 0.85 + (0.03 - 0.85)/(1+EXP(-0.0007*(hsurf(:,:)-5000.)))
     !RH8
!    DO k=1, nlay-1
!        relhum(:,:,:) = 0.92 + (0.03 - 0.92)/(1+EXP(-0.0007*(zml(:,:,:)-5000.)))
!    END DO
!    rhsurf(:,:) = 0.92 + (0.03 - 0.92)/(1+EXP(-0.0007*(hsurf(:,:)-5000.)))
     !RH10
!    DO k=1, nlay-1
!        relhum(:,:,:) = 0.8 + (0.03 - 0.8)/(1+EXP(-0.0007*(zml(:,:,:)-5000.)))
!    END DO
!    rhsurf(:,:) = 0.8 + (0.03 - 0.8)/(1+EXP(-0.0007*(hsurf(:,:)-5000.)))
