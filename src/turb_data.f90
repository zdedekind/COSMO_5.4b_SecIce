...
#elif __COSMO__
  tkhmin       =  0.40_wp,   & ! scalar (heat) transport
  tkmmin       =  0.40_wp,   & ! momentum transport
#endif
...
#elif __COSMO__
  pat_len      = 500.0_wp,   & ! effective length scale of subscale surface patterns over land [m]
#endif
...
! 6. Parameters which are computed during model run
!--------------------------------------------------
...
REAL (KIND=wp)     ::        &
  ...
  ! these parameters are physical constants which are either taken as are
  ! or set to 0.0 in the turbulence for special applications
  tur_rcpv,         & ! cp_v/cp_d - 1
  tur_rcpl            ! cp_l/cp_d - 1 (where cp_l=cv_l)


! 7. Definition of used types
!----------------------------
...
! 8. Parameters for organization of local arrays
!    (which fields are in which layer of variable "a"
!----------------------------------------------------
...
! 9. Local fields, which have to be allocated in extra routines
!--------------------------------------------------------------

! for turb_transfer and turb_diffusion
!$OMP threadprivate (grad, l_scal, fc_min, tmps, vaps, prss, eprs, liqs)
...
!$OMP threadprivate (rho_tar, exner_tar, diss_tar, shflu_s_tar, qvflu_s_tar)
REAL (KIND=wp), ALLOCATABLE, TARGET :: &
     !    memory-target for pointer declared in organize_turbdiff:
     rho_tar  (:,:),  &
     exner_tar(:,:),  &
     diss_tar (:,:),  &
     shflu_s_tar(:),  &
...
! for turbtran
!$OMP threadprivate (h_top_2d, h_atm_2d, edh, z0m_2d, z0d_2d, z2m_2d, z10m_2d,    &
!$OMP                hk_2d, hk1_2d, h_can_2d, rat_m_2d, rat_h_2d, fac_h_2d,       &
!$OMP                fac_m_2d, frc_2d, vel_2d, ts_2d, qds_2d,  dz_sg_m, dz_sg_h,  &
!$OMP                dz_g0_m, dz_g0_h, dz_0a_m, dz_0a_h, dz_sa_h, dz_s0_h,        &
!$OMP                velmin, ratsea)
REAL (KIND=wp), ALLOCATABLE, TARGET ::  &
...
!$OMP threadprivate (rclc,  tketens_tar, k_2d, lo_ice)
REAL (KIND=wp), ALLOCATABLE, TARGET :: &
     rclc   (:,:),    & ! cloud cover
...
! for turbdiff
!$OMP threadprivate (vari, dicke, len_scale, hor_scale, rhon,                     &
!$OMP                shv, frh, frm, ftm, a, hlp, can,                             &
!$OMP                lay, lays, xri, vars, dzsm, dzsh, src,                       &
!$OMP                cbig_tar, csml_tar, rair_tar)
REAL (KIND=wp), ALLOCATABLE, TARGET :: &
...
LOGICAL :: &
  ...
  l3dturb       =.FALSE., & ! a model run with 3D-(turbulent)-diffusion
...
#elif __COSMO__
  imode_tran    =1, & ! mode of TKE-equation in transfer scheme             (compare 'imode_turb')
#endif
...
#elif __COSMO__
  icldm_tran    =0, & ! mode of cloud representation in transfer parametr.  (compare 'icldm_turb')
#endif
...
#ifdef __COSMO__
! still used for the old version of the scheme
REAL (KIND=wp)     ::     &
...
! for organize_turbdiff
  ...
  ALLOCATE ( grad         (nmvar)                    , STAT=istat)

  ALLOCATE ( rho_tar      (nproma,ke)                , STAT=istat)
  ALLOCATE ( exner_tar    (nproma,ke)                , STAT=istat)
  ALLOCATE ( diss_tar     (nproma,kez1)               , STAT=istat)
  ALLOCATE ( shflu_s_tar  (nproma)                   , STAT=istat)
  ALLOCATE ( qvflu_s_tar  (nproma)                   , STAT=istat)
...
! for turbtran
  ...
  ALLOCATE ( ts_2d        (nproma)                   , STAT=istat)
  ALLOCATE ( qds_2d       (nproma)                   , STAT=istat)
...
!US  ALLOCATE ( vari         (nproma,ke-1:kez1,ndim)     , STAT=istat)
!US also in turbdiff, for whole atmosphere: use this one
!US  ALLOCATE ( a            (nproma, kez1:kez1,ndim)     , STAT=istat)
  ALLOCATE ( rclc         (nproma, kez1:kez1)          , STAT=istat)
!US  ALLOCATE ( len_scale    (nproma, kez1:kez1)          , STAT=istat)
...
!US also in turbdiff, for whole atmosphere: use this one
!US  ALLOCATE ( frm          (nproma, kez1:kez1)          , STAT=istat)
!US  ALLOCATE ( frh          (nproma, kez1:kez1)          , STAT=istat)
!US  ALLOCATE ( rhon         (nproma, kez1:kez1)          , STAT=istat)

! for turbdiff
  ALLOCATE ( vari         (nproma,kez1,ndim)          , STAT=istat)
  ...
  ALLOCATE ( a            (nproma,kez1,ndim)          , STAT=istat)
  ...
  ALLOCATE ( vars         (nproma,2)                 , STAT=istat)
  ALLOCATE ( dzsm         (nproma)                   , STAT=istat)
  ALLOCATE ( dzsh         (nproma)                   , STAT=istat)
  ALLOCATE ( src          (nproma)                   , STAT=istat)

  ALLOCATE ( cbig_tar     (nproma,kcm:kez1)           , STAT=istat)
  ALLOCATE ( csml_tar     (nproma,kcm:kez1)           , STAT=istat)
  ALLOCATE ( rair_tar     (nproma,kcm-1:kez1)         , STAT=istat)
...
! for organize_turbdiff
  ...
  DEALLOCATE ( rho_tar        , STAT=istat)
  DEALLOCATE ( exner_tar      , STAT=istat)
  DEALLOCATE ( diss_tar       , STAT=istat)
  DEALLOCATE ( shflu_s_tar    , STAT=istat)
  DEALLOCATE ( qvflu_s_tar    , STAT=istat)
...
! for turbtran
  ...
  DEALLOCATE ( ts_2d          , STAT=istat)
  DEALLOCATE ( qds_2d         , STAT=istat)
...
!US  DEALLOCATE ( vari           , STAT=istat)
!US also in turbdiff, for whole atmosphere: use this one
!US  DEALLOCATE ( a              , STAT=istat)
  DEALLOCATE ( rclc           , STAT=istat)
!US  DEALLOCATE ( len_scale      , STAT=istat)
...
!US also in turbdiff, for whole atmosphere: use this one
!US  DEALLOCATE ( frm            , STAT=istat)
!US  DEALLOCATE ( frh            , STAT=istat)
!US  DEALLOCATE ( rhon           , STAT=istat)
! for turbdiff
  DEALLOCATE ( vari           , STAT=istat)
  ...
  DEALLOCATE ( vars           , STAT=istat)
  DEALLOCATE ( dzsm           , STAT=istat)
  DEALLOCATE ( dzsh           , STAT=istat)
  DEALLOCATE ( src            , STAT=istat)

  DEALLOCATE ( cbig_tar       , STAT=istat)
  DEALLOCATE ( csml_tar       , STAT=istat)
  DEALLOCATE ( rair_tar       , STAT=istat)
