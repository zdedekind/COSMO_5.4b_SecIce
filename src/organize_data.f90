...
!  Adaptations to call modified write_ready_final from io_utilities 
!    (unification with INT2LM)
! V5_3         2015-10-09 Ulrich Schaettler, Ulrich Blahak, Astrid Kerkweg
!  Write HHL to restart files: this is necessary when starting from GRIB2 files
!   and does not hurt otherwise (US)
!  Moved computation of ndiff_bd_ini out of src_input.f90, so it can be computed 
!   once here for all cases (including restarts).
!  Set loutput_diab to .TRUE., if TTENS_DIAB occurs in some output list (UB)
!  Bug fix for setting next output step in restart runs (AK)
! V5_3a        2015-11-24 Ulrich Blahak, KIT
!  Corrections for ASCII output in YUSPECIF (UB)
!  Activate calls to art_restart_fields (KIT)
! V5_4         2016-03-10 Ulrich Schaettler
!  Bug fix: replaced VORTIC1, VORTIC2, VORTIC3 with VORTIC_U, VORTIC_V, VORTIC_W
! V5_4a        2016-05-10 Matthias Raschendorfer
!  'SSO_STDH' is included in the list for initial data, external data and the
!  output list for analysis, if itype_vdif > -2, also if SSO is not run.
!  This field is used in the new COSMO-ICON turbulence scheme.
!
! Code Description:
! Language: Fortran 90.
...
USE data_runcontrol,    ONLY:                               &
    ...
    loutput_diab, & ! output of diabatic tendencies is requested
...
USE utilities,          ONLY:  diff_minutes
...
CHARACTER (LEN=3)          :: yzhead_ready ! head for ready files
...
!                                                              yvarbd(n),'C10'
  ...
  ! constant fields:
  ! since GRIB2 is possible, write HHL in addition to HSURF into the restart file
  ! in order not to recompute it after restarting. We need both, HSURF and HHL,
  ! because HSURF is packed with a different precision in GRIB2, so their values 
  ! are really different
  pp_restart%yvarml( 1)  = 'HSURF     '
  pp_restart%yvarml( 2)  = 'HHL       '
  pp_restart%yvarml( 3)  = 'FR_LAND   '
  pp_restart%yvarml( 4)  = 'Z0        '
  pp_restart%yvarml( 5)  = 'SOILTYP   '
  pp_restart%yvarml( 6)  = 'PLCOV     '
  pp_restart%yvarml( 7)  = 'LAI       '
  pp_restart%yvarml( 8)  = 'ROOTDP    '
  pp_restart%yvarml( 9)  = 'VIO3      '
  pp_restart%yvarml(10)  = 'HMO3      '

  ! prognostic variables and dynamics:
  pp_restart%yvarml(11)  = 'U         '
  pp_restart%yvarml(12)  = 'V         '
  pp_restart%yvarml(13)  = 'W         '
  pp_restart%yvarml(14)  = 'T         '
  pp_restart%yvarml(15)  = 'PP        '
  pp_restart%yvarml(16)  = 'T_SNOW    '
  pp_restart%yvarml(17)  = 'W_I       '
  pp_restart%yvarml(18)  = 'QV_S      '
  pp_restart%yvarml(19)  = 'W_SNOW    '
  pp_restart%yvarml(20)  = 'T_S       '

  ! fields from the parameterizations
  ! microphysics:  nothing special

  ! radiation:
  pp_restart%yvarml(21)  = 'SOHR_RAD  '
  pp_restart%yvarml(22)  = 'THHR_RAD  '
  pp_restart%yvarml(23)  = 'ALB_RAD   '
  pp_restart%yvarml(24)  = 'SOBS_RAD  '
  pp_restart%yvarml(25)  = 'THBS_RAD  '
  pp_restart%yvarml(26)  = 'PABS_RAD  '
  pp_restart%yvarml(27)  = 'SOBT_RAD  '
  pp_restart%yvarml(28)  = 'THBT_RAD  '
  pp_restart%yvarml(29)  = 'CLCH      '
  pp_restart%yvarml(30)  = 'CLCM      '
  pp_restart%yvarml(31)  = 'CLCL      '
  pp_restart%yvarml(32)  = 'CLCT      '

  ! turbulence:
  pp_restart%yvarml(33)  = 'TKVM      '
  pp_restart%yvarml(34)  = 'TKVH      '
  pp_restart%yvarml(35)  = 'TCM       '
  pp_restart%yvarml(36)  = 'TCH       '

  ! convection:
  pp_restart%yvarml(37)  = 'CLC_CON   '
  pp_restart%yvarml(38)  = 'CLW_CON   '
  pp_restart%yvarml(39)  = 'PRR_CON   '
  pp_restart%yvarml(40)  = 'PRS_CON   '
  pp_restart%yvarml(41)  = 'TOP_CON   '
  pp_restart%yvarml(42)  = 'BAS_CON   '
  pp_restart%yvarml(43)  = 'DU_CON    '
  pp_restart%yvarml(44)  = 'DV_CON    '
  pp_restart%yvarml(45)  = 'DT_CON    '
  pp_restart%yvarml(46)  = 'DQV_CON   '
  pp_restart%yvarml(47)  = 'DQC_CON   '
  pp_restart%yvarml(48)  = 'DQI_CON   '
  pp_restart%yvarml(49)  = 'MFLX_CON  '
  pp_restart%yvarml(50)  = 'CAPE_CON  '
  pp_restart%yvarml(51)  = 'QCVG_CON  '
  pp_restart%yvarml(52)  = 'TKE_CON   '

  ! soil model:

  ! and all the rest:
  pp_restart%yvarml(53)  = 'TMAX_2M   '
  pp_restart%yvarml(54)  = 'VMAX_10M  '
  pp_restart%yvarml(55)  = 'TMIN_2M   '
  pp_restart%yvarml(56)  = 'RUNOFF_S  '
  pp_restart%yvarml(57)  = 'RUNOFF_G  '
  pp_restart%yvarml(58)  = 'ASOB_S    '
  pp_restart%yvarml(59)  = 'ATHB_S    '
  pp_restart%yvarml(60)  = 'ASOB_T    '
  pp_restart%yvarml(61)  = 'ATHB_T    '
  pp_restart%yvarml(62)  = 'ALHFL_S   '
  pp_restart%yvarml(63)  = 'ASHFL_S   '
  pp_restart%yvarml(64)  = 'AUMFL_S   '
  pp_restart%yvarml(65)  = 'AVMFL_S   '
  pp_restart%yvarml(66)  = 'APAB_S    '
  pp_restart%yvarml(67)  = 'DQVDT     '
  pp_restart%yvarml(68)  = 'QVSFLX    '
  pp_restart%yvarml(69)  = 'QRS       '
  pp_restart%yvarml(70)  = 'SNOW_GSP  '
  pp_restart%yvarml(71)  = 'RAIN_GSP  '
  pp_restart%yvarml(72)  = 'RAIN_CON  '
  pp_restart%yvarml(73)  = 'SNOW_CON  '
  pp_restart%yvarml(74)  = 'PRR_GSP   '
  pp_restart%yvarml(75)  = 'PRS_GSP   '
  pp_restart%yvarml(76)  = 'T_G       '
  pp_restart%yvarml(77)  = 'PS        '
  pp_restart%yvarml(78)  = 'VGUST_CON '
  pp_restart%yvarml(79)  = 'VGUST_DYN '
  pp_restart%yvarml(80)  = 'SOD_T     '
  pp_restart%yvarml(81)  = 'ASOD_T    '
  pp_restart%yvarml(82)  = 'VABSMX_10M'
  pp_restart%yvarml(83)  = 'LHFL_S    '
  pp_restart%yvarml(84)  = 'SHFL_S    '
  pp_restart%yvarml(85)  = 'SNOW_MELT '
  pp_restart%nyvar_m     = 85
 ...
 iout = 0
  ...
  gribout_loop: DO
    ...
    IF ((nstep == now%ngrib(now%nextstep) .AND. L_BM_ORIG_OUTPUT) &
      ...
      ! (But ready files are only written if all GRIBOUT groups are finished. 
      !  if there is a mix of forecast and analysis groups, the name of the 
      !  ready-file might not be correct)
  ...
  IF (lwriteready) THEN
    CALL write_ready_final(nstep, dt, yzhead_ready,                   &
...
IF (my_world_id == 0) THEN
  ...
  ENDIF
   ...
   IF (lsso) THEN
     yvarc_d (nzmxc_d + 1)  = 'SSO_STDH  '
     yvarc_d (nzmxc_d + 2)  = 'SSO_GAMMA '
     yvarc_d (nzmxc_d + 3)  = 'SSO_THETA '
     yvarc_d (nzmxc_d + 4)  = 'SSO_SIGMA '
     nzmxc_d = nzmxc_d + 4
   ELSEIF (itype_vdif > -2) THEN
     yvarc_d (nzmxc_d + 1)  = 'SSO_STDH  '
     nzmxc_d = nzmxc_d + 1
...
!   ENDDO
    ...
    outblock%ysuffix                                = TRIM(charbuf(4))
    outblock%ytunit                                 = TRIM(charbuf(5))
    outblock%ydomain                                = TRIM(charbuf(6))
...
DO n = 1, outblock%nyvar_z

  SELECT CASE (TRIM(outblock%yvarzl(n)))
  CASE ('TTENS_DIAB')
    loutput_diab = .TRUE.
  END SELECT
...
DO n = 1, outblock%nyvar_p

  SELECT CASE (TRIM(outblock%yvarpl(n)))
  CASE ('TTENS_DIAB')
    loutput_diab = .TRUE.
  END SELECT
...
ELSEIF (nstart >= outblock%ngrib(outblock%outsteps)) THEN
  ! then nothing has to be written for this namelist group in the restart run
   ! leads to problems when last output is < nstart in restarts!!
   !  outblock%nextstep = nstop + 1  ! leads to problems when last output is < nstart in restarts!!
   ! this is the better choice
  outblock%nextstep = outblock%outsteps
ELSE
  outblock%nextstep = 1
...
IF (my_world_id == 0) THEN
  ...
  WRITE(nuspecif,'(T8,A,T33,I12,T52,I12,T71,A)')                          &
              'nprocess_ini',outblock%nprocess_ini_out,nprocess_ini_d,'I'
  WRITE(nuspecif,'(T8,A,T33,I12,T52,I12,T71,A)')                          &
                 'nprocess_bd',outblock%nprocess_bd_out,nprocess_bd_d,'I'
  WRITE(nuspecif,'(T8,A,T33,I12,T52,I12,T71,A)')                          &
