...
!   (some have been forgotten before)
! V5_4a        2016-05-10 Matthias Raschendorfer
!  Use new module turb_data instead of data_turbulence
! V5_4b        2016-07-12 Jochen Foerstner
!  Added new namelist switches for Tiedtke-Bechtold (IFS) convection and
!   use all convection data from new module conv_data.f90
!
! Code Description:
! Language: Fortran 90.
...
!OCH >>
    lcmprates,    & ! .TRUE.: allocate cmp rates and calculate
!OCH <<
...
USE conv_data,       ONLY :   &
    ...
    tune_capdcfac_et, & ! fraction of CAPE diurnal cycle correction applied in the extratropics
    tune_rhebc_land,  & ! RH threshold for onset of evaporation below cloud base over land
    tune_rhebc_ocean, & ! RH threshold for onset of evaporation below cloud base over sea
    tune_texc,        & ! excess value for temperature used in test parcel ascent
    tune_qexc,        & ! excess fraction of grid-scale QV used in test parcel ascent
    tune_rcucov,      & ! convective area fraction
    tune_entrorg,     & ! entrainment parameter for deep convection valid at dx=20 km   
    tune_rhebc_land_trop,  & ! RH threshold for onset of evaporation below cloud base over land in the tropics
    tune_rhebc_ocean_trop, & ! RH threshold for onset of evaporation below cloud base over sea in the tropics
    tune_rcucov_trop         ! convective area fraction in the tropics
...
! Variables for default values
  ...
  LOGICAL                    ::       &
    ...
    l2dim_d,             & ! lartif_data=.TRUE.:  2dimensional model version
                           !            =.FALSE.: full 3dimensional version
!OCH >>
    lcmprates_d            ! micropysical process rates
!OCH<<
...
!                        'lcmprates'    , lcmprates    , lcmprates_d  , ' L '
  !OCH
  WRITE (nuspecif, '(A2)')  '  '
...
! Variables for default values
  REAL (KIND=wp)             ::       &
    ...
    tune_capdcfac_et_d, & ! fraction of CAPE diurnal cycle correction applied in the extratropics
    tune_rhebc_land_d,  & ! RH threshold for onset of evaporation below cloud base over land
    tune_rhebc_ocean_d, & ! RH threshold for onset of evaporation below cloud base over sea
    tune_texc_d,        & ! excess value for temperature used in test parcel ascent
    tune_qexc_d,        & ! excess fraction of grid-scale QV used in test parcel ascent
    tune_rcucov_d,      & ! convective area fraction
    tune_entrorg_d,     & ! entrainment parameter for deep convection valid at dx=20 km   
    tune_rhebc_land_trop_d,  & ! RH threshold for onset of evaporation below cloud base over land in the tropics
    tune_rhebc_ocean_trop_d, & ! RH threshold for onset of evaporation below cloud base over sea in the tropics
    tune_rcucov_trop_d,      & ! convective area fraction in the tropics
  ...
  NAMELIST /TUNING/ rlam_mom, rlam_heat, rat_sea, rat_lam, rat_can,           &
                    ...
                    tune_capdcfac_et, tune_rhebc_land, tune_rhebc_ocean,      &
                    tune_texc, tune_qexc, tune_rcucov, tune_entrorg,          &
                    tune_rhebc_land_trop, tune_rhebc_ocean_trop,              &
                    tune_rcucov_trop
...
!------------------------------------------------------------------------------
  ...
  ! Initial values taken from turb_data
  rlam_mom_d   = rlam_mom
  ...
  entr_sc_d    = entr_sc
  thick_sc_d   = thick_sc
  tune_capdcfac_et_d = tune_capdcfac_et
  tune_rhebc_land_d  = tune_rhebc_land
  tune_rhebc_ocean_d = tune_rhebc_ocean
  tune_texc_d        = tune_texc
  tune_qexc_d        = tune_qexc
  tune_rcucov_d      = tune_rcucov
  tune_entrorg_d     = tune_entrorg
  tune_rhebc_land_trop_d  = tune_rhebc_land_trop
  tune_rhebc_ocean_trop_d = tune_rhebc_ocean_trop
  tune_rcucov_trop_d      = tune_rcucov_trop
...
IF (nproc > 1) THEN
  ...
  IF (my_world_id == 0) THEN
    ...
    ! tuning parameters for Tiedtke-Bechtold IFS-/ICON-convection
    realbuf(38) = tune_capdcfac_et
    realbuf(39) = tune_rhebc_land
    realbuf(40) = tune_rhebc_ocean
    realbuf(41) = tune_texc
    realbuf(42) = tune_qexc
    realbuf(43) = tune_rcucov
    realbuf(44) = tune_entrorg
    realbuf(45) = tune_rhebc_land_trop
    realbuf(46) = tune_rhebc_ocean_trop
    realbuf(47) = tune_rcucov_trop
  ENDIF

  CALL distribute_values (realbuf, 47, 0, imp_reals,    icomm_world, ierr)
  ...
  IF (my_world_id /= 0) THEN
    ...
    ! tuning parameters for Tiedtke-Bechtold IFS-/ICON-convection
    tune_capdcfac_et = realbuf(38)
    tune_rhebc_land  = realbuf(39)
    tune_rhebc_ocean = realbuf(40)
    tune_texc        = realbuf(41)
    tune_qexc        = realbuf(42)
    tune_rcucov      = realbuf(43)
    tune_entrorg     = realbuf(44)
    tune_rhebc_land_trop  = realbuf(45)
    tune_rhebc_ocean_trop = realbuf(46)
    tune_rcucov_trop      = realbuf(47)
  ENDIF
...
IF (my_world_id == 0) THEN
  ...
  ! tuning parameters for Tiedtke-Bechtold IFS-/ICON-convection
  WRITE (nuspecif, '(T8,A,T33,F12.4,T52,F12.4,T71,A3)')                      &
          'tune_capdcfac_et', tune_capdcfac_et, tune_capdcfac_et_d, ' R '
  WRITE (nuspecif, '(T8,A,T33,F12.4,T52,F12.4,T71,A3)')                      &
          'tune_rhebc_land',  tune_rhebc_land,  tune_rhebc_land_d,  ' R '
  WRITE (nuspecif, '(T8,A,T33,F12.4,T52,F12.4,T71,A3)')                      &
          'tune_rhebc_ocean', tune_rhebc_ocean, tune_rhebc_ocean_d, ' R '
  WRITE (nuspecif, '(T8,A,T33,F12.4,T52,F12.4,T71,A3)')                      &
          'tune_texc',        tune_texc,        tune_texc_d,        ' R '
  WRITE (nuspecif, '(T8,A,T33,F12.4,T52,F12.4,T71,A3)')                      &
          'tune_qexc',        tune_qexc,        tune_qexc_d,        ' R '
  WRITE (nuspecif, '(T8,A,T33,F12.4,T52,F12.4,T71,A3)')                      &
          'tune_rcucov',      tune_rcucov,      tune_rcucov_d,      ' R '
  WRITE (nuspecif, '(T8,A,T33,F12.4,T52,F12.4,T71,A3)')                      &
          'tune_entrorg',     tune_entrorg,     tune_entrorg_d,     ' R '
  WRITE (nuspecif, '(T8,A,T33,F12.4,T52,F12.4,T71,A3)')                      &
    'tune_rhebc_land_trop', tune_rhebc_land_trop, tune_rhebc_land_trop_d,  ' R '
  WRITE (nuspecif, '(T8,A,T33,F12.4,T52,F12.4,T71,A3)')                      &
    'tune_rhebc_ocean_trop', tune_rhebc_ocean_trop, tune_rhebc_ocean_trop_d, ' R '
  WRITE (nuspecif, '(T8,A,T33,F12.4,T52,F12.4,T71,A3)')                      &
          'tune_rcucov_trop', tune_rcucov_trop, tune_rcucov_trop_d, ' R '
  WRITE (nuspecif, '(A2)')  '  '
ENDIF
