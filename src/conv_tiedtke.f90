...
!
! Description:
!   The module "conv_tiedtke" performs calculations related to the 
!   parameterization of subgrid-scale moist (deep and shallow) convection. 
!   The present parameterization scheme is a mass-flux scheme according to 
!   Tiedtke (1989).
!
!   All global fields of the model that are used by the convection routines
!   are passed via argument lists in blocked data format, while scalars
...
!
! History from former module src_conv_tiedtke:
! --------------------------------------------
...
USE conv_data, ONLY:  &
    ...
    lmfmid ,      & ! switch for mid-level convection
    lmfdd  ,      & ! switch for inclusion of downdrafts
    lmfdudv,      & ! switch for cumulus effects on momentum

    entrpen,      & ! mean entrainment rate for deep convection
    entrmid,      & ! mean entrainment rate for mid-level convection
    entrdd ,      & ! mean entrainment rate for downdrafts
    entr_pb,      & ! constant in extended formulation (DM)
                    ! turbulent entrainment/detrainment rate

    cmfcmax,      & ! maximum mass flux
    cmfcmin,      & ! minimum mass flux  (security)
    cmfctop,      & ! relative mass flux above level of non-buoyancy
    cmfdeps,      & ! relative mass flux at level of free-sinking

    capeconstant, & ! time-constant cu_base massflux from CAPE
    capemin,      & ! minimum CAPE for cu_base massflux determination
    cmbtke ,      & ! time-constant cu_base massflux from ctke
    ctkemin,      & ! mimimum ctke for cu_base massflux determination

    cprcon ,      & ! conversion rate cloud water to precipitation

    Tmpmin ,      & ! Minium temperature of the mixed-phase temperature range [K]
    Tmpmax ,      & ! Maximum temperature of the mixed-phase temperature range [K]
    exp_mp ,      & ! Exponent in the interpolation formula
...
#ifdef _OPENACC
USE conv_data, ONLY:                                                        &
    mlab   , mclab  , mtype  , mdtop  , mctop0 , mlwmin , mtmelt , kcptop , &
    ztu    , zqu    , zuu    , zvu    , zlu    , zlude  , zmfu   , zmfus  , &
    zmfuq  , zmful  , zdmfup , ztd    , zqd    , zud    , zvd    , zmfd   , &
    zmfds  , zmfdq  , zdmfdp , zcptu  , zcpqu  , zmfuu  , zmfuv  , zmfdu  , &
    zmfdv  , zqsen  , ztenh  , zqenh  , zqsenh , zrhgdz , zcape  , z1dp   , &
    zcond  , zqold  , zrfl   , zrneva , zentr  , zhcbase, zmfub  , zmfub1 , &
    zdqpbl , zdqcv  , zwmax  , zqenwb , ztenwb , ztru   , ztrd   , zmfutr , &
    zmfdtr
#endif
!------------------------------------------------------------------------------
...
!==============================================================================
!+ Module procedure to compute Tiedtke convection
!------------------------------------------------------------------------------

! cu_tiedtke with new output arguments
!    zdtdiab_con (conv T-tend. due to LH exchanges)
!    zdtlhn_con  (conv T-tend. due to LH exchanges, without rain evap.)
! cu_tiedtke with new arguments for cloud-water and cloud-ice tendencies due to 
! detrained convective cloud condensate (DM)

SUBROUTINE cu_tiedtke (                                          &
           pt     , pqv     , ptrcr   , pu    , pv    , pw    ,  &
           ptke   , pfif    , pfih    , ppf   , pph   ,          &
           pt_g   , pdqvdt  , pqhfl   , lolp  , pdt2  ,          &
           nvec   , kdim    , isc     , iec   , nztr  , iblock,  &
           pcumflx, pcucape , pcuconv , pcuctke,                 &
           pvddraf, pco_clw ,                                    &
...
! Description:
!
!   The module procedure cu_tiedtke organizes the massflux cumulus parameterization
!   scheme according to M. Tiedke.
...
! ----------
  ...
  REAL(KIND=wp), INTENT (IN)  ::  &
     ...
     ptrcr      (nvec,kdim,nztr), & ! tracers at full levels
     pu         (nvec,kdim),      & ! zonal wind component
     pv         (nvec,kdim),      & ! meridional wind component
     pw         (nvec,kdim),      & ! vertical velocity in z-system (full levles)
     ptke       (nvec,kdim),      & ! turbulent kinetic energy (SQRT(2*TKE))
     ...
     pt_g       (nvec),           & ! surface temperature
     pdqvdt     (nvec,kdim),      & ! moisture tencency
     pqhfl      (nvec),           & ! surface moisture flux    
     pdt2                           ! timestep (*2 for leapfrog integration)

  LOGICAL, INTENT (IN)        ::  &
     lolp       (nvec)              ! land-sea indicator
  
! Output data
! -----------
  REAL(KIND=wp), INTENT(OUT)  ::  &
     pcumflx    (nvec),           & ! cu_base massflux
     pcucape    (nvec),           & ! convective available energy (J/kg)
     pcuconv    (nvec),           & ! moisture convergence used for cu_base massflux
     pcuctke    (nvec),           & ! convective turbulent energy (J/kg)
     pvddraf    (nvec),           & ! maximum possible convective gust  (was INOUT before????)
     ...
     ptrcr_con  (nvec,kdim,nztr), & ! convective tendency of tracers
     pdtke_con  (nvec,kdim),      & ! convective boyant TKE prpoduction on half levels
     pqct_con   (nvec,kdim),      & ! convective tendency of cloud water
     pqit_con   (nvec,kdim),      & ! convective tendency of cloud ice
     pdu_con    (nvec,kdim),      & ! convective tendency of u                 
     pdv_con    (nvec,kdim),      & ! convective tendency of v                 
     ppr_con    (nvec),           & ! convective precipitation rate of rain
     pps_con    (nvec),           & ! convective precipitation rate of snow
     ppne_con   (nvec),           & ! convective precipitation rate without evaporation
     pdtdiab_con(nvec,kdim),      & ! conv T-tend. due to LH exchanges
     pdtlhn_con (nvec,kdim)         ! conv T-tend. due to LH exchanges/no rain evap.
...
! ----------------------------------
  ...
  INTEGER ::  &
    ...
    mctop0 (nvec),            & !
    mlwmin (nvec),            & !
    mtmelt (nvec),            & ! melting level
    kcptop (nvec)               ! highest unstable layer in CAPE-determination
  ...
  REAL(KIND=wp)           ::  &
    ...
    zmfdq  (nvec,kdim) ,      & !
    zdmfdp (nvec,kdim) ,      & !
    zcptu  (nvec,kdim) ,      & ! updraft temperature in CAPE-determination
    zcpqu  (nvec,kdim) ,      & ! updraft humidity in CAPE-determination
    zmfuu  (nvec,kdim) ,      & ! mass flux up: u
    zmfuv  (nvec,kdim) ,      & ! mass flux up: v
    zmfdu  (nvec,kdim) ,      & ! mass flux down: u
    zmfdv  (nvec,kdim) ,      & ! mass flux down: v
  ...
  REAL(KIND=wp)           ::  &
    zcape  (nvec)      ,      & ! convective available energy CAPE
    z1dp   (nvec)      ,      & !
    zcond  (nvec)      ,      & !
    zqold  (nvec)      ,      & !
    zrfl   (nvec)      ,      & !
  ...
  REAL(KIND=wp)           ::  &
    ztru   (nvec,kdim,nztr),  & ! tracer updraft
    ztrd   (nvec,kdim,nztr),  & ! tracer downdraft
    zmfutr (nvec,kdim,nztr),  & ! mass flux up: tracers
    zmfdtr (nvec,kdim,nztr)     ! mass flux down: tracers
#endif
...
#ifdef MESSY
  REAL(KIND=wp)     :: xdpevap, xsfl(nvec), xrfl(nvec), xrsum, xpsubcl(nvec) &
...
! ----------
  ...
  REAL    (KIND=wp   ),     INTENT (IN) ::  &
     pt     (nvec,kdim),     & ! temperature at full levels
     pth    (nvec,kdim),     & ! temperature at half levels
     pqv    (nvec,kdim),     & ! specific humidity at full levels
     pqvh   (nvec,kdim),     & ! specific humidity at half levels
     ptr    (nvec,kdim,nztr),& ! tracers at full levels
     pu     (nvec,kdim),     & ! zonal wind component at full levels
     pv     (nvec,kdim),     & ! meridional wind component at full levels
     pw     (nvec,kdim),     & ! vertical velocity of z-system
     pph    (nvec, kdim+1),  & ! pressure at half levels 
     ppf    (nvec,kdim  ),   & ! pressure at full levels 
     pfif   (nvec,kdim  ),   & ! geopotential at full levels
     pfih   (nvec,kdim  ),   & ! geopotential at half levels
     pqsen  (nvec,kdim  ),   & ! sat. specific humidity     
     pdqvdt (nvec,kdim  ),   & ! humidity tendency          
     pdqpbl (nvec       )      ! pbl moisture convergence

  REAL    (KIND=wp   ),     INTENT (INOUT) ::  &
     pmf_ub (nvec     ), &    ! upward mass flux at cloud base
     pmf_u  (nvec,kdim), &    ! upward mass flux
     pmf_us (nvec,kdim), &    ! upward mass flux *s
     pmf_uq (nvec,kdim), &    ! upward   * qv
     pmf_ul (nvec,kdim), &    ! upward   * ql
     pzumflx(nvec     ), &    !
     plude  (nvec,kdim), &    ! 
     pdmfup (nvec,kdim), &    !
     pentr  (nvec     ), &    ! entrainment 
     pt_u   (nvec,kdim), &    !
     pqv_u  (nvec,kdim), &    !
     plu    (nvec,kdim), &    !
     ptr_u  (nvec,kdim,nztr),&!   
     pu_u   (nvec,kdim), &    ! 
     pv_u   (nvec,kdim)       !
  ...
  INTEGER, INTENT (INOUT) ::  &
     ktype  (nvec),    &  !
     mbas_con(nvec),   &  ! indicator for convection (on/off) at gridpoints
     mtop_con(nvec),   &  ! indicator for convection (on/off) at gridpoints
     mtop_con0(nvec),  &  ! indicator for convection (on/off) at gridpoints
     klab(nvec,kdim)      ! indicator for convection (on/off) at gridpoints

  LOGICAL                 , INTENT (IN) ::  &
     lolp (nvec)          ! land-sea indicator
  LOGICAL                 , INTENT (INOUT) ::  &
     locum(nvec)          ! indicator for convection (on/off) at gridpoints
...
! ----------
  ...
  LOGICAL                 , INTENT (INOUT) ::  &
     loddraf(nvec)          ! switch for downdrafts
  
! Output data
! -----------
  REAL    (KIND=wp   ),     INTENT (INOUT) ::  &
     pt_d   (nvec, kdim),       & ! downdraft temperature
     pqv_d  (nvec, kdim),       & ! downdraft specific humidity
     ptr_d  (nvec, kdim, nztr), & ! downdraft tracers
     pu_d   (nvec, kdim),       & ! downdraft u-velocity
     pv_d   (nvec, kdim),       & ! downdraft v-velocity
     pvddraf(nvec      ),       & ! maximum possible convective gust
     prfl   (nvec      ),       & ! total precipitation flux
                                  !     various mass fluxes
     pmf_d   (nvec, kdim),      & !
     pmf_ds  (nvec, kdim),      & ! 
     pmf_dq  (nvec, kdim),      & !
...
! ----------------------------------
  ...
  REAL    (KIND=wp   )     ::  &
    zvbuo  (nvec)            ! gusts generated by buoyancy forces
...
!
  DO i=isc,iec
    pvddraf(i) = SQRT(0.2_wp*MAX(zvbuo(i),0.0_wp))   !US then we do not need to set it to 0.0   + pvddraf(i))
...
! ----------
  ...
  REAL     (KIND=wp   ),     INTENT (IN) ::  &
     p1op  (nvec)    ! reciprocal of pressure, 1.0/p

  LOGICAL                 ,  INTENT (IN) ::  &
     plflag (nvec),& ! switch for points where adjustment shall be made
...
! -----------
  REAL     (KIND=wp   ),     INTENT (INOUT) ::  &
     pt    (nvec), & ! temperature on input, adjusted on output
     pqv   (nvec)    ! specific humidity on input, adjusted on ouput
...
! ----------------------------------
  ...
  REAL    (KIND=wp   )     ::  &
    zcond(nvec)      ! condensation amount
...
!------------------------------------------------------------------------------
! End of module conv_tiedtke
!------------------------------------------------------------------------------
END MODULE conv_tiedtke
