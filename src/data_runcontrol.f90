...
! --------------------------
  ...
  INTEGER   (KIND=iintegers)       ::           &
    ...
    itype_vdif,   & ! type of vertical diffusion calculation
                    ! < 0: vertical diffusion calculated by the specific COSMO-routines 
                    !      in the slow tendencies modules
                    !  -2: apply former (non-blocked) COSMO-version of Raschendorfer-scheme
                    !      (only temporary and meaningful, if "itype_turb=3)
                    ! >=0: vertical diffusion calculated by the common COSMO/ICON-routine 
                    !      contained in MODULE 'turb_turbdiff' 
                    !      (not yet completely implemented, only if "itype_turb=3")
                    !   0: vertical diffusion within the CALL of the turbulence model (ICON-mode)
                    ! > 0: vertical diffusion CALLed at the end of physics-section and 
                    !      considering explicit tendencies in the semi-implicit solution.

    imode_mdif,   & ! mode of momentum diffusion (in case of "itype_vdif>0")
                    !   1: extra call for diffusion of each wind-component applied to the 
                    !      staggered wind positions
                    !   0: diffusion of momentum together with that of scalars on horiz. 
                    !      mass-positions and never using any explicit wind-tendencies in 
                    !      implicit diffusion equation
                    !  -1: as " 0", but using the expl. wind-tend. already present at 
                    !      mass-positions (if requested)
                    !  -2: as "-1", but using even the staggered wind-tend. (after horiz. 
                    !      interp. and if requested)
  ...
  LOGICAL                          ::           &
    ...
    !OCH>>
    lcmprates     ! write output of microphysical process rates
    !OCH<<
